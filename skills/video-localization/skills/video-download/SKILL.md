...
- Writes `<name>.source.json` itself from the probe result (the safe way — the old `--dump-json > file.json` stdout redirect silently swallowed downloads, and yt-dlp's `print_to_file` broke on newer versions writing a 3-byte "NA")
...
**Quality override:** by default cook takes the best quality the source offers. If the user asked for less ("1080p is fine", "skip 4K"), pass `--quality 1080` (caps height; falls back to the best muxed stream at or under that height). Only cap when the user asks. Note: `-F` listings on HLS sources estimate size from the manifest's PEAK bandwidth — a downloaded file 2-3x smaller than the estimate is normal (VBR average vs peak), not a wrong-stream signal.

**Multi-language sources (auto-dubbed audio tracks).** YouTube channels with auto-dubbing enabled serve 20+ audio-language variants per resolution, HLS-only (no separate video/audio streams). Two consequences: (1) cook's format spec carries a muxed `/b` fallback so the default path works on these sources; (2) yt-dlp's sort prefers the ORIGINAL track but nothing asserts it — read `cargo` and `cargo_warnings` in cook's output JSON. A **language** warning (`audio language 'X' != original 'Y'`) means a dub track was grabbed — re-download with explicit stream selection; a **height** warning means the `--quality` cap couldn't be honored — re-pick or accept and note it for the user. Untagged audio (no language tag on either side) leaves the language unverifiable — treat that as a known blind spot, not a pass. A wrong-language download caught here costs a re-download; caught after transcription it costs the entire pipeline.

Done when `cook download` exits 0 and the JSON output reports `duration > 0`, `thumbnail_renamed: true`, `source_json_present: true`, and `cargo_warnings: []` — plus `cargo.audio_language` present and non-null. If `audio_language` is absent or null (untagged audio or a probe error), the language is unverified: say so in the handoff rather than calling the check passed. If `thumbnail_renamed` is false or `source_json_present` is false, tell the user explicitly — these are degraded-completion artifacts (some sources have no thumbnail; a transient JSON failure shouldn't block the download) but downstream will need to know (frame-grab cover later, metadata re-fetch).
